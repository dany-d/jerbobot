...
/**
* Feedback controller setpoint written to by traject_new and read by the
* controller.
*/
...
int main(int argc, char *argv[]) {
	...
	*/
	if (rc_filter_pid(&D5, D5_KP, D5_KI, D5_KD, 4 * DT, DT)) {
		fprintf(stderr, "ERROR in jb_main, failed to make filter D5\n");
		return -1;
	}
	...
	*/
	...
	trajec_mat.d[0][2] = 0; // pos_y_r 0
	trajec_mat.d[0][3] = 0; // pos z 0 
	trajec_mat.d[1][0] = 2; // time 1
	trajec_mat.d[1][1] = 0; 
	trajec_mat.d[1][2] = 0;
	trajec_mat.d[1][3] = 10;
	trajec_mat.d[2][0] = 4; // time 2
	trajec_mat.d[2][1] = 0; 
	trajec_mat.d[2][2] = 0;
	trajec_mat.d[2][3] = 20;
	trajec_mat.d[3][0] = 6; // time 3
	trajec_mat.d[3][1] = 0;
	trajec_mat.d[3][2] = 0;
	trajec_mat.d[3][3] = 0;
...
static void __traject_new(void) {	
	...
	double t_ay = ((cstate.t_2 - cstate.t_1 -
		pow(pow((cstate.t_2 - cstate.t_1), 2) - 4 * z_diff / ACCEL_MAX, .5)) / 2);

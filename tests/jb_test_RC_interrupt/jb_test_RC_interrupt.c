...
static void* __RC_reader(void* ptr);		///< background thread
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	if(rc_pthread_create(&rc_read_thread, __RC_reader, (void*) NULL, SCHED_OTHER, 0)){
 ...
 */
 ...
 /*
	Loop that check RC signal for inputs.
	R: 
	M:
	E:

 */
 static void* __RC_reader(__attribute__((unused)) void* ptr)
 {
 	double FB_drive_stick, LR_drive_stick, arm_drive_stick, e_stop_switch; // for input sticks
 	int FB_CH = 4;
 	int LR_CH = 3;
 	int arm_CH = 1;
 	int e_stop_CH = 5;
